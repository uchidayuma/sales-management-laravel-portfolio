<form id="quotationForm" class='quotation-form br5' action="{{ route('quotations.store', ['id' => $contact->id] ) }}" method="POST">
  @csrf
  ...
  <div class='mb10 d-flex justify-content-between'>
    <div class='client-wrapper w45per'>
      <h4 class='quotation-target h4 bold'><input type='text' class='form-control' value="{{ isCompany($contact) ? $contact->company_name : $contact->surname.$contact->name }} {{ isCompany($contact) ? '御中' : '様' }}" placeholder="見積もり相手の名前" name='q[client_name]'></h4>
  ...
  <input type="submit" class="btn btn-info align-right" id="post-quotation" value="{{ strpos(url()->current(),'edit') === false ? '見積もりを作成' : 'コピーして見積もりを作成' }}">
</form>